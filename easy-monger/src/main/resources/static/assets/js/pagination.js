(function ( $ ) {
        	...
        	$(this).children('ul').click(
				function(event) {
					...
					if($position > 0 && $position < ($totalPage>5?6:$totalPage+1) && $totalPage > 1) {
						...
						debugger;
						$parentTag.find('ul > li').eq($position).addClass('active');
						$parentTag.find('ul > li').eq($position).siblings().removeClass('active');
					}
			});
        });
 	}
    $.fn.Pagination = function( options, callback ) {
        ...
        return this.each( function() {
			...
			var current = settings.page ? settings.page : 1;

        	//$(this).children('ul').children('li').eq(settings.from).addClass('active').siblings().removeClass('active');
			$(this).children('ul').children('li').eq(current).addClass("active");
			$(this).children('ul').children('li').eq(current).siblings().removeClass('active');
